    ...
    <div className="animate-in slide-in-from-bottom-4 fade-in duration-300 h-full flex flex-col">
      ...
      {loading ? (
        <div className="flex-1 flex flex-col items-center justify-center min-h-[400px] border border-dashed border-border rounded-xl bg-surface/50">
            ...
            <p className="font-mono text-sm text-gray-400 animate-pulse text-center">
                Menulis {kisiList.length} butir soal...<br/>
                <span className="text-xs text-gray-500">Menganalisis stimulus & memvalidasi konstruksi</span>
            </p>
        </div>
      ) : error ? (
                    ...
                    return (
                        <button 
                            ...
                            className={`p-3 rounded-lg border text-left transition-all relative overflow-hidden group
                                ${activeTab === idx 
                                    ? 'bg-surface2 border-accent shadow-md' 
                                    : 'bg-surface border-border hover:border-gray-600'}
                            `}
                        >
                            <div className="flex justify-between items-center mb-1">
                                <span className={`font-mono font-bold ${activeTab === idx ? 'text-white' : 'text-gray-400'}`}>No {s.no}</span>
                                <div className={`w-2 h-2 rounded-full ${statusColor}`} />
                            </div>
                            <div className="hidden md:block text-[10px] text-gray-500 truncate">{s.stem}</div>
                             {s.image_prompt && (
                                <div className="hidden md:block absolute top-1 right-1">
                                    <span className="text-[8px] bg-blue-500/20 text-blue-300 px-1 rounded">IMG</span>
                                </div>
                            )}
                        </button>
                    )
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-2">
                            <div className="px-3 py-1 bg-surface3 rounded border border-border flex items-center gap-2 whitespace-nowrap">
                                <span className="text-xs font-mono text-gray-400">QA Score:</span>
                            ...
                            {currentSoal.validasi.map((v, i) => (
                                <div key={i} className={`px-3 py-1 rounded border flex items-center gap-2 text-xs whitespace-nowrap
                                    ${v.status === 'PASS' ? 'bg-green-500/10 border-green-500/20 text-green-400' :
                                      v.status === 'WARN' ? 'bg-yellow-500/10 border-yellow-500/20 text-yellow-400' :
                        ...
                        <div className="bg-surface border border-border rounded-xl p-6">
                            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-border">
                                ...
                                <div className="text-xs text-gray-400 font-mono">Level: <span className="text-white">{currentSoal.level}</span></div>
                                <div className="text-xs text-gray-400 font-mono">Kesukaran: <span className="text-white">{currentSoal.kesukaran}</span></div>
                            </div>

                            {/* Stimulus Section (Text & Image) */}
                            <div className="mb-6 p-4 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                                <div className="text-[10px] uppercase font-bold text-blue-400 mb-2 tracking-widest">Stimulus</div>
                                
                                {currentSoal.stimulus && (
                                    <p className="text-sm text-gray-300 leading-relaxed whitespace-pre-line mb-3">{currentSoal.stimulus}</p>
                                )}
                                        ...
                                        {currentSoal.image_data ? (
                                            <div className="relative group">
                                                ...
                                                <button 
                                                    ...
                                                    className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 bg-black/70 text-white text-xs px-2 py-1 rounded transition-opacity"
                                                >
                                                    Regenerate
                                                </button>
                                            </div>
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-xs text-gray-400 italic">Soal ini memerlukan gambar/ilustrasi.</span>
                                                    <button 
                                                        onClick={() => handleGenerateImage(activeTab, currentSoal.image_prompt!)}
                                                ...
                                                <div className="p-2 bg-surface3/50 rounded text-[10px] font-mono text-gray-500 truncate">
                            ...
                            <div className="mb-6">
                                <p className="text-base font-medium text-white leading-relaxed whitespace-pre-line">{currentSoal.stem}</p>
                            </div>
                                    ...
                                    {Object.entries(currentSoal.opsi).map(([key, val]) => {
                                        ...
                                        return (
                                            <div key={key} className={`flex gap-3 p-3 rounded-lg border text-sm transition-colors
                                                ${isKey ? 'bg-green-500/10 border-green-500/30 text-green-300' : 'bg-surface2 border-transparent text-gray-400'}
                                            `}>
                                                <span className="font-mono font-bold w-4">{key}.</span>
                                                <span>{val}</span>
                                                {isKey && <span className="ml-auto text-[10px] font-bold uppercase tracking-wider bg-green-500 text-white px-2 py-0.5 rounded">Kunci</span>}
                                            </div>
                                        )
                                    })}
                                </div>
                            ...
                            <div className="mt-8 pt-6 border-t border-border">
                                <h4 className="text-xs font-bold text-gray-500 uppercase mb-2">Pembahasan</h4>
                                <p className="text-sm text-gray-400 leading-relaxed whitespace-pre-line">{currentSoal.pembahasan}</p>
                            </div>
                            
                            {currentSoal.rubrik && (
                                <div className="mt-6 pt-6 border-t border-border">
                                    <h4 className="text-xs font-bold text-gray-500 uppercase mb-2">Rubrik Penilaian</h4>
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-xs text-left">
                                            <thead className="text-gray-500 font-mono border-b border-border">
                                                <tr>
                                                    <th className="py-2">Aspek</th>
                                                    <th className="py-2">Deskriptor</th>
                                            ...
                                            <tbody className="text-gray-300">
                                                {currentSoal.rubrik.map((r, ri) => (
                                                    <tr key={ri} className="border-b border-border/50">
                                                        <td className="py-2 pr-4 align-top">{r.aspek}</td>
                                                        <td className="py-2 pr-4 text-gray-400">{r.deskriptor}</td>
                                                        <td className="py-2 text-right font-mono align-top text-accent2">{r.skor}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
