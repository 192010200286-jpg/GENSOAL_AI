  ...
  return (
    <div className="animate-in slide-in-from-bottom-4 fade-in duration-300 h-full flex flex-col">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-1 bg-surface border border-border rounded-xl p-6 h-fit shadow-sm">
            <h3 className="text-xs font-bold text-gray-700 uppercase tracking-widest font-mono mb-6">Ringkasan</h3>
            <div className="space-y-4">
                <div className="flex justify-between items-center border-b border-border pb-3">
                    <span className="text-gray-800 text-sm font-medium">Jumlah Soal</span>
                    <span className="text-black font-mono font-bold">{soalList.length} Butir</span>
                </div>
                <div className="flex justify-between items-center border-b border-border pb-3">
                    <span className="text-gray-800 text-sm font-medium">Estimasi Waktu</span>
                    <span className="text-black font-mono font-bold">{ctx.waktu} Menit</span>
                </div>
                <div className="flex justify-between items-center border-b border-border pb-3">
                    <span className="text-gray-800 text-sm font-medium">Rata-rata Kualitas</span>
                ...
                <div className="flex justify-between items-center pb-1">
                    <span className="text-gray-800 text-sm font-medium">Status Validasi</span>
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
             ...
             <button 
                ...
                className="group bg-surface2 border border-border hover:border-blue-500 p-6 rounded-xl text-left transition-all hover:bg-white hover:shadow-md"
             >
                <div className="text-2xl mb-3 text-blue-700">📊</div>
                <div className="font-bold text-black mb-1">Unduh Kisi-Kisi (.doc)</div>
                <div className="text-xs text-gray-700 group-hover:text-black font-medium">File tabel kisi-kisi rapi, siap print.</div>
             </button>

             {/* Download Soal */}
             <button 
                onClick={() => downloadDoc(`SOAL_${ctx.mapel}_${ctx.kelas}.doc`, () => generateSoalHTML(true))}
                className="group bg-surface2 border border-border hover:border-green-500 p-6 rounded-xl text-left transition-all hover:bg-white hover:shadow-md"
             >
                <div className="text-2xl mb-3 text-green-700">📝</div>
                <div className="font-bold text-black mb-1">Unduh Naskah Soal (.doc)</div>
                <div className="text-xs text-gray-700 group-hover:text-black font-medium">File soal + gambar + kunci & pembahasan.</div>
             </button>

             {/* Download All */}
             <button 
                onClick={() => downloadDoc(`PAKET_LENGKAP_${ctx.mapel}_${ctx.kelas}.doc`, generateFullDoc)}
                className="md:col-span-2 group bg-surface2 border border-border hover:border-accent p-4 rounded-xl text-center transition-all hover:bg-white hover:shadow-md mt-2 flex flex-col items-center justify-center gap-2"
             >
                <span className="text-2xl">📁</span>
                <span className="font-bold text-gray-900 group-hover:text-accent text-sm">Unduh Paket Lengkap (Kisi + Soal) .doc</span>
                <span className="text-[10px] text-gray-600 font-bold">Kompatibel dengan MS Word & Google Docs</span>
             </button>

             {/* Copy Button (Secondary) */}
             <button onClick={handleCopy} className="md:col-span-2 bg-transparent border border-dashed border-border hover:border-accent p-3 rounded-lg flex items-center justify-center gap-2 text-gray-700 hover:text-accent font-bold transition-all mt-2">
